...
{    /// <summary>   
    ...
    {
        //数据库连接字符串(web.config来配置)System.Web.HttpContext.Current.Server.MapPath(ConfigurationManager.AppSettings["AccessConnectionString"])
        public static string a = string.Format("Provider = Microsoft.Jet.OLEDB.4.0; Persist Security Info=True;Data Source =|DataDirectory|\\{0};", ConfigurationManager.AppSettings["AccessConnectionString"]);
        public static string connectionString = a;
        public DbHelperACE()
        {
        }
        ...
        public static DataTable QueryTable(string SQLString, params OleDbParameter[] cmdParms)
        {
            using (OleDbConnection connection = new OleDbConnection(connectionString))
            {
                OleDbCommand cmd = new OleDbCommand();
